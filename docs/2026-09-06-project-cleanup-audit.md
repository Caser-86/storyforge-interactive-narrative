...
- 后续复核说明：本文保留 2026-09-06 的原始审计结论；文末“2026-09-15 复核补充”用于覆盖后续验证和本轮清理扫描结果。
...
- 全局清理扫描未发现可在不询问作者的情况下删除的源码、依赖、资源、测试或配置文件。没有因为“搜索不到直接引用”而删除 B 级或 C 级文件。
- `output/`、`.next/`、`.next-playwright/`、`test-results/`、`.superpowers/`、`.agents/` 和 `data/` 均属于被忽略的构建、工具、本地证据或作者数据目录；当前发布包和 checkpoint 仍有用途，因此保留，不将其误判为垃圾。
- 根目录 `PROJECT_*.md`、`IMPROVEMENTS_CHECKLIST.md` 和 `LOCAL_PERSISTENCE_DESIGN_AND_AUDIT.md` 已由 `docs/README.md` 标记为历史资料；它们含退役架构描述但仍有追溯价值，本轮保留。
- `public/.gitkeep` 是 Dockerfile 静态资源复制契约所需的占位文件；不存在运行时图片、音频或字体资源需要清理。
- `NEXT_PUBLIC_APP_URL` 仅出现在被忽略的本地 `.env.local` 和历史路线图文字中，当前源码与 `.env.example` 均不读取它；为保护用户本地配置，本轮不修改或删除该变量。
- 后续隔离 Docker build/health smoke 已通过；这覆盖容器构建和本地健康检查，不等同于目标环境部署或正式签名发布。
- 本轮最新验证：`npm run verify` 为 86 个测试文件 / 433 个测试通过，`npm run test:e2e:authoring` 为 17/17；`git diff --check` 通过。当前候选仍未提交、推送、打标签或发布。
