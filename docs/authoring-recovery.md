...
互动调用会写入本地用量账本，只保存任务、尝试、模型、请求 ID、状态、token 数和耗时，不保存 API key、原始 prompt 或原始模型响应。设置 `STORYFORGE_MAX_OUTPUT_TOKENS` 后，预算按单个互动会话累计；请求超时且服务端无法确认 token 时，预留额会进入 `unknown`，需要作者检查配置或重新开始，不会被当作免费调用。编辑器生成作者结局使用独立账本记录确认用量、失败和未知预算，不占用互动会话预算；如果要完整恢复这些账本记录，请使用 SQLite checkpoint。
