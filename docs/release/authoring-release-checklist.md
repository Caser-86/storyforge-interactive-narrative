...
- [ ] Real-model semantic scoring for the final release model is complete and matches `OPENAI_MODEL`; the AI-assisted pre-review is available, but the four scores and the `zh-suspense-16` ending-intent decision are still author actions.
...
- [x] The AI-assisted semantic pre-review was synchronized through [PR #8](https://github.com/Caser-86/storyforge-interactive-narrative/pull/8), merged to `master` as `06deca9`; post-merge CI run [35378189219](https://github.com/Caser-86/storyforge-interactive-narrative/actions/runs/35378189219) passed. This does not replace the formal author score.
...
- [x] 2026-09-19 final live structural recheck: with process-level `OPENAI_MODEL=deepseek-v4-flash`, all three approved fixtures ultimately passed (`6/6`, `8/8`, `16/16`) with ending, choice-contract, risk-coverage, and consequence checks passing and `issueCodes=[]`. The 16-turn fixture first stopped at `14/16` with `GENERATION_SCHEMA`/`RISK_SEQUENCE` and recovered on the bounded rerun; this is recorded as transient model drift, not hidden as a pass. Human four-dimension scoring and final release-model confirmation remain unchecked.
- [x] 2026-09-19 final-config model structural recheck: the current `.env.local` model `doubao-seed-evolving` passed provider smoke and all three approved live fixtures (`6/6`, `8/8`, `16/16`) with ending, choice-contract, risk-coverage, and consequence checks passing and `issueCodes=[]`; the saved review artifacts passed the secret-marker scan. This closes the model-match automation evidence only; human four-dimension scoring remains unchecked.
