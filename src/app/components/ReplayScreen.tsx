"use client";

import { useState, useEffect } from "react";
import { useGameStore } from "@/lib/store";

interface ReplayScene {
  turn: number;
  title: string;
  location: string;
  timeOfDay: string;
  mood: string[];
  body: string;
  npcs: Array<{ name: string; role: string; dialogue: string }>;
  chosenLabel: string | null;
}

export default function ReplayScreen({ sessionId }: { sessionId: string }) {
  const [scenes, setScenes] = useState<ReplayScene[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const { reset, ownerToken } = useGameStore();

  useEffect(() => {
    const headers: Record<string, string> = {};
    if (ownerToken) headers["x-owner-token"] = ownerToken;

    fetch(`/api/games/${sessionId}`, { headers })
      .then((r) => {
        if (!r.ok) throw new Error(r.status === 403 ? "无权访问此故事" : "故事未找到");
        return r.json();
      })
      .then((data: { scenes?: Array<{ turn: number; title: string; location: string; time_of_day: string; mood: string[] | string; body: string; npcs: Array<{ name: string; role: string; dialogue: string }> | string }> }) => {
        const mapped = (data.scenes || []).map((s): ReplayScene => ({
          turn: s.turn,
          title: s.title,
          location: s.location,
          timeOfDay: s.time_of_day,
          mood: Array.isArray(s.mood) ? s.mood : [],
          body: s.body || "",
          npcs: Array.isArray(s.npcs) ? s.npcs : [],
          chosenLabel: null,
        }));
        setScenes(mapped);
        setLoading(false);
      })
      .catch((err) => {
        setError(err.message);
        setLoading(false);
      });
  }, [sessionId, ownerToken]);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0a0a1a] via-[#1a1a2e] to-[#16213e]">
        <div className="w-8 h-8 border-2 border-[#e94560] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }

  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0a0a1a] via-[#1a1a2e] to-[#16213e]">
        <div className="text-center">
          <p className="text-red-400 mb-4">{error}</p>
          <button onClick={reset} className="text-[#e94560] hover:underline">
            返回首页
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a1a] via-[#1a1a2e] to-[#16213e] p-4 sm:p-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-bold text-white">故事回放</h1>
          <button onClick={reset} className="text-gray-400 hover:text-white text-sm">
            ← 新故事
          </button>
        </div>

        <div className="space-y-6">
          {scenes.map((scene, i) => (
            <div key={i} className="scene-card">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs px-2 py-0.5 rounded bg-[#e94560]/20 text-[#e94560]">
                  第 {scene.turn} 幕
                </span>
                <span className="text-gray-400 text-xs">
                  📍 {scene.location} · 🕐 {scene.timeOfDay}
                </span>
              </div>
              <h2 className="text-lg font-bold text-white mb-2">{scene.title}</h2>
              <p className="text-gray-200 leading-relaxed whitespace-pre-wrap text-sm">
                {scene.body}
              </p>
              {scene.npcs.length > 0 && (
                <div className="mt-3 space-y-2">
                  {scene.npcs.map((npc, j) => (
                    <div key={j} className="npc-card bg-[#1a1a2e]/80 border border-[#333] rounded-lg p-3">
                      <span className="text-[#e94560] text-sm font-semibold">{npc.name}</span>
                      <span className="text-gray-500 text-xs ml-2">{npc.role}</span>
                      <p className="text-gray-300 italic text-xs mt-1">&ldquo;{npc.dialogue}&rdquo;</p>
                    </div>
                  ))}
                </div>
              )}
              {scene.chosenLabel && (
                <div className="mt-3 text-xs text-[#e94560]">
                  选择 → {scene.chosenLabel}
                </div>
              )}
            </div>
          ))}
        </div>

        <div className="text-center mt-8 text-gray-500 text-sm">— 故事结束 —</div>
      </div>
    </div>
  );
}
