    ...
    <main className="editor-shell-page">
      ...
      <div className="editor-workspace">
        ...
        <section className="editor-main-panel" aria-label="节点编辑区域">
          {selectedNode ? (
            <NodeCanvas projectId={project.id} node={selectedNode} graph={draftGraph} onSaved={(updatedNode) => setDraftGraph((current) => ({ ...current, nodes: current.nodes.map((node) => node.id === updatedNode.id ? updatedNode : node) }))} />
          ) : (
...
function NodeCanvas({ projectId, node, graph, onSaved }: { projectId: string; node: StoryNode; graph: StoryGraph; onSaved: (node: StoryNode) => void }) {
