...
vi.mock("@/lib/db", () => ({
  ...
  withTransaction: vi.fn(async (callback: (tx: { query: ReturnType<typeof vi.fn> }) => Promise<unknown>) => {
    const mockTx = {
      query: vi.fn().mockResolvedValue({ rows: [{ id: "test" }], duration: 1 }),
    };
...
import { POST as postChoice } from "@/app/api/games/[sessionId]/choices/route";
import { GET as getSession, PATCH as patchSession, DELETE as deleteSession } from "@/app/api/games/[sessionId]/route";

function mockRequest(body: Record<string, unknown>, method = "POST"): Request {
  return new Request("http://localhost:3000/api/games/sess_test/choices", {
    method,
...
function mockParams(sessionId: string) {
  return Promise.resolve({ sessionId });
}

describe("POST /api/games/[sessionId]/choices", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("returns 400 when sceneId or choiceId is missing", async () => {
    const req = mockRequest({});
    const res = await postChoice(req, { params: mockParams("sess_test") });
  ...
  it("returns 404 when session not found", async () => {
    ...
    const req = mockRequest({ sceneId: "scene_1", choiceId: "choice_1" });
    const res = await postChoice(req, { params: mockParams("sess_nonexist") });
  ...
  it("returns 400 when session is not active", async () => {
    (query as ReturnType<typeof vi.fn>).mockImplementation((sql: string) => {
      if (sql.includes("game_sessions")) {
        return { rows: [{ id: "sess_test", status: "ended", state_json: "{}", language: "zh-CN", rating: "PG-13" }], duration: 0 };
      }
...
describe("GET /api/games/[sessionId]", () => {
  ...
  it("returns 404 when session not found", async () => {
    (query as ReturnType<typeof vi.fn>).mockResolvedValue({ rows: [], duration: 0 });
    const req = new Request("http://localhost:3000/api/games/sess_nonexist");
    const res = await getSession(req, { params: mockParams("sess_nonexist") });
    expect(res.status).toBe(404);
  });
  ...
  it("returns session details when found", async () => {
    (query as ReturnType<typeof vi.fn>).mockImplementation((sql: string) => {
      if (sql.includes("FROM game_sessions")) {
        return {
          rows: [{
            id: "sess_test", seed_prompt: "test", genre: "fantasy",
            language: "zh-CN", rating: "PG-13", status: "active",
            current_scene_id: "scene_1", state_json: "{}",
    ...
    const req = new Request("http://localhost:3000/api/games/sess_test");
    const res = await getSession(req, { params: mockParams("sess_test") });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.session.id).toBe("sess_test");
    expect(body.session.status).toBe("active");
  });
});

describe("PATCH /api/games/[sessionId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 400 for invalid status", async () => {
    ...
    const res = await patchSession(req, { params: mockParams("sess_test") });
    expect(res.status).toBe(400);
  });
  ...
  it("returns 404 when session not found for patch", async () => {
    ...
    const res = await patchSession(req, { params: mockParams("sess_nonexist") });
    expect(res.status).toBe(404);
  });
});
...
describe("DELETE /api/games/[sessionId]", () => {
  ...
  it("returns 404 when session not found for delete", async () => {
    (query as ReturnType<typeof vi.fn>).mockResolvedValue({ rows: [], duration: 0 });
    const req = new Request("http://localhost:3000/api/games/sess_nonexist", { method: "DELETE" });
    const res = await deleteSession(req, { params: mockParams("sess_nonexist") });
    expect(res.status).toBe(404);
  });
  ...
  it("deletes session successfully", async () => {
    ...
    const req = new Request("http://localhost:3000/api/games/sess_test", { method: "DELETE" });
    const res = await deleteSession(req, { params: mockParams("sess_test") });
    expect(res.status).toBe(200);
    const body = await res.json();
