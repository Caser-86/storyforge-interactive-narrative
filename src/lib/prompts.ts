...
输出 JSON Schema（必须严格遵循）：
{
  "scene": {
    ...
    "choices": [{
      ...
      "route": "investigate / act / social / stealth / sacrifice（路线类型，可选）",
...
选择分支差异规则（最重要）：
- 3 个选择必须分别代表不同的剧情方向和路线类型：
  1. 调查路线(investigate)：低风险，偏向收集信息、观察、准备。
  2. 行动路线(act)：中风险，偏向直接行动、突破、推进。
  3. 社交/其他路线(social/stealth/sacrifice)：高风险，偏向交涉、潜行、或付出代价。
- 每个选择应附带 route 字段标明路线类型。
- 至少1个选择必须推进主线目标。
- 至少1个选择必须暴露代价(有负面的 stateEffects)。
...
export function buildUserPrompt(params: {
  ...
  if (params.storyPhase) {
    sections.push(`当前故事阶段：\n${params.storyPhase}`);
  }

  if (params.targetTurns && params.remainingTurns !== undefined) {
    sections.push(`故事进度：\n目标${params.targetTurns}步，剩余${params.remainingTurns}步`);
  }

  if (params.phaseInstruction) {
    sections.push(`阶段指令（必须遵守）：\n${params.phaseInstruction}`);
  }

  if (params.mustResolveThreads) {
    sections.push(`强制要求：必须回收至少1个旧伏笔，禁止引入新主线伏笔。`);
  } else if (params.allowNewThreads === false) {
    sections.push(`限制：当前阶段不建议引入新伏笔。`);
  }
